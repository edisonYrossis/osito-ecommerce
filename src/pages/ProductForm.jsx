...
const handleSubmit = async (e) => {
  ...
  const db = getFirestore(app)
    ...
    alert('product agregado ID', docref.id)
...
useEffect( () => {
  ...
  const getDocList = async () => {
    ...
    const allProductsArray = querySnapshot.docs.map(doc => doc.data())

    setDocsList(allProductsArray)
...
useEffect(() => {
  ...
  const getDocList = async () => {
    ...
    const allProducts = await docsList
...
const newPrice = calcPrice(parseFloat(price), productType, productWeight, productGain)
  ...
  <>
    ...
    <div className={`h-5/6 w-full md:w-3/4 flex md:inline-flex  justify-center items-center `}>
         <div className='w-5/6 h-5/6 bg-slate-200 shadow-md rounded-md flex flex-col justify-evenly items-center p-1'>
